import {
  ...
  restartAnimation,
} from "./scripts/animation-controls.js";
import {
  toggleMenu,
  changeTheme,
  bodyCloseMenu,
...
function isNumberUsedInGlobo(globoIndex, number) {
  return predefinedOrder.some(
    (round, roundIndex) =>
      roundIndex !== currentRound && round[globoIndex] === number
  );
}
...
document.getElementById("prevRound").addEventListener("click", () => {
  if (currentRound > 0) {
    currentRound--;
    document.getElementById("roundDisplay").textContent = `Rodada ${
      currentRound + 1
    }`;
    updateButtons();
  }
...
document
  .getElementById("stateButton")
  .addEventListener("click", toggleStateAnimation);
document
